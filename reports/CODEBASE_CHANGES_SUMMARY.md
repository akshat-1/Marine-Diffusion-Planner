...
3. **Hybrid Waypoint Loss Alignment**: Applied explicit $\Delta t = 10.0\text{s}$ timestep scaling to velocity integration and fixed target waypoint feature selection.
4. **Exponential Moving Average (EMA)**: Added EMA weight updating ($\mathrm{decay}=0.999$) and checkpointing to stabilize diffusion training.
...
#### Problem:
CommonOcean stores vessel velocities in the vessel's local body frame (`state.velocity` = surge $u$, `state.velocity_y` = sway $v$). The previous implementation treated $(u, v)$ directly as world-frame Cartesian velocities and rotated them by $-\theta_{\mathrm{ego}}$, creating a **double rotation** bug that corrupted velocity features.
...
2. The integrated waypoint calculation omitted the timestep $\Delta t = 10.0\text{s}$, scaling integrated spatial waypoints incorrectly by a factor of 10.
...
- Separated spatial waypoints $(x, y)$ from velocity targets $(v_x, v_y, \theta, r)$:
  ...
  ```
- Scaled integrated velocity waypoints by $\Delta t = 10.0\text{s}$:
  $$\hat{\tau}_\theta^x = \mathrm{detached\_integral}(\hat{v}_\theta, W=3) \cdot \Delta t \quad (\text{where } \Delta t = 10.0\text{s})$$
- Total loss:
...
2. **Ego First-Frame Outlier Filter**:
   Added displacement check in `_build_index_map()` to skip sliding windows where distance between $t_{\mathrm{start}}$ and $t_{\mathrm{anchor}}$ exceeds $8000.0\text{m}$.
