...
\begin{frame}{Coastline Resampling \& Feature Tensors}
    ...
    \begin{block}{Output Feature Tensors per Sample (10D Feature Representation)}
        ...
        \texttt{ego_target} & $(20, 10)$ & Ground-truth target trajectory \\
        \bottomrule
        \end{tabular}
        \end{table}
    \end{block}
...
\begin{frame}{10D External Force \& Drift Physics Modeling}
    \begin{block}{Why Force Conditioning is Essential}
        When ships drift due to wind or waves, Course Over Ground ($\mathrm{COG}$) differs from Heading ($\psi$). Unconditioned models mistake environmental leeway drift for intentional steering.
    \end{block}

    \begin{block}{MMG 3-DOF Inverse Dynamics Residue Equation}
        We compute the mass-normalized specific external force residual vector $\mathbf{f}_{\mathrm{ext}}$:
        $$\mathbf{f}_{\mathrm{ext}} = \left[ \frac{F_{x,\mathrm{ext}}}{m}, \; \frac{F_{y,\mathrm{ext}}}{m}, \; \frac{N_{\mathrm{ext}}}{m \cdot L}, \; \beta_{\mathrm{drift}} \right]$$
        where $F_{y,\mathrm{ext}} = (m+m_y)\dot{v} + (m+m_x)ur - F_{y,\mathrm{hull}}$ and $\beta_{\mathrm{drift}} = \mathrm{COG} - \psi$.
    \end{block}

    \begin{itemize}
        \item \textbf{Why Specific Force Wins}: Mass-normalized acceleration ($\text{m/s}^2$) brings small boats and container ships into unit scale $\sim [-1, 1]$, enabling \textbf{fast neural network convergence} while preserving 2nd-order acceleration physics ($\dot{u}, \dot{v}, \dot{r}$).
    \end{itemize}
\end{frame}

\begin{frame}{Stratified Spatial \& Traffic Density Sampling}
    \begin{block}{Spatial Grid-Cell Binning (\texttt{MAX\_SCENARIOS\_PER_GRID_CELL = 100})}
...
\begin{frame}{Scenario Quality Ranking Engine (\texttt{validate\_scenerios.py})}
    \begin{block}{0--100 Numerical Scenario Quality Score}
        Evaluates and ranks generated scenario XMLs across 5 maritime metrics:
        $$\mathrm{Quality\ Score} = \mathrm{Integrity}(30) + \mathrm{Density}(25) + \mathrm{Maneuver}(20) + \mathrm{Encounter}(15) + \mathrm{Environment}(10)$$
    \end{block}

    \begin{block}{Quality Tier Classification}
        \begin{itemize}
            \item \textbf{Tier S (Superior, $\ge 85$ pts)}: Rich multi-ship encounters, tactical turns, coastlines, 0 integrity issues.
            \item \textbf{Tier A (Advanced, $70 - 84$ pts)}: Multi-ship tactical navigation scenes.
            \item \textbf{Tier B / C (Baseline / Low Quality)}: Standard transit / low density.
            \item \textbf{Tier F (Failed, $0$ pts)}: Hard collision, NaNs, or shallow water grounding.
        \end{itemize}
    \end{block}

    \begin{itemize}
        \item \textbf{CLI Top-K Exporter}: \texttt{--top-k 500 --export-dir ./top_scenarios/} copies top-ranked XMLs automatically.
    \end{itemize}
\end{frame}
...
\begin{frame}{Feature Normalization: Z-Score Standard Normalization}
    ...
    \begin{block}{Feature Channel Normalization Statistics}
        \begin{itemize}
            \item \textbf{Ego/Agent State Features} $[x, y, v_x, v_y, \theta, r]$:
              $$\mu_{\mathrm{state}} = [0, 0, 3.5, 0, 0, 0], \quad \sigma_{\mathrm{state}} = [100.0, 100.0, 3.0, 1.0, 1.0, 0.05]$$
