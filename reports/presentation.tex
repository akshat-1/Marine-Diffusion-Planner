...
\begin{frame}{Physical \& Hydrodynamic Constants in Force Calculations}
    ...
    Yaw Added Moment & $J_z / (m L^2)$ & $0.025$ & Added inertia moment ratio \\
...
\begin{frame}{Second-Order Mean Wave Drift Mechanics \& HDP Pipeline Usage}
    \begin{block}{1. Wave Physics: First-Order vs. Second-Order Mean Drift}
        \begin{itemize}
            \item \textbf{First-Order Wave Action}: Oscillates at wave frequency (5--10s period), causing roll/pitch. Averages to \textbf{zero} over 200s trajectory horizon.
            \item \textbf{Second-Order Mean Wave Drift ($F_{y,\mathrm{wave}}$)}: Non-linear wave diffraction; exerts a \textbf{steady, non-zero lateral push} that constantly drifts hull off course.
        \end{itemize}
    \end{block}

    \begin{block}{2. Mass-Normalized Feature Pipeline (\texttt{preparedataset.py})}
        Combined with wind drag into lateral residual acceleration $a_{y,\mathrm{ext}}$:
        $$a_{y,\mathrm{ext}} = \frac{F_{y,\mathrm{ext}}}{m} = \frac{F_{y,\mathrm{wind}} + F_{y,\mathrm{wave}}}{m} \quad (\text{channel 7 of 10D State Vector})$$
    \end{block}
...
\begin{frame}{Scenario Quality Ranking Engine (\texttt{validate\_scenarios.py})}
    ...
    \begin{itemize}
        \item \textbf{CLI Top-K Exporter}: \texttt{--top-k 500 --export-dir ./top\_scenarios/} copies top-ranked XMLs automatically.
    \end{itemize}
\end{frame}
