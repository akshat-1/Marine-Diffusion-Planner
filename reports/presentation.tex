...
\begin{frame}{Mathematical Derivation: Significant Wave Height ($H_s$) Extraction}
    \begin{block}{1. Wave Elevation Energy Spectrum Definition}
        Significant wave height $H_s$ is four times the square root of the zeroth spectral moment $m_0$:
        $$H_s = 4 \sigma_z = 4 \sqrt{m_0} = 4 \sqrt{\int_0^\infty S(\omega) d\omega}$$
    \end{block}

    \begin{block}{2. Inverse Hydrodynamic Derivation from AIS Kinematics}
        From Maruo's 2nd-order mean wave drift equation:
        $$F_{y,\mathrm{wave}} = \frac{1}{2} \rho_w g H_s^2 L C_{Dwy}(\chi)$$
        Subtracting wind drag $F_{y,\mathrm{wind}}$ from total MMG residual $F_{y,\mathrm{ext}} = (m+m_y)\dot{v} + (m+m_x)ur - F_{y,\mathrm{hull}}$ isolates $F_{y,\mathrm{wave}}$. Rearranging yields the \textbf{effective equivalent wave height $H_s$}:
        $$H_s = \sqrt{\frac{2 \cdot |F_{y,\mathrm{ext}} - F_{y,\mathrm{wind}}|}{\rho_w \cdot g \cdot L \cdot C_{Dwy}(\chi)}}$$
    \end{block}
\end{frame}
